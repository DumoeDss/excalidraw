...
If deployment was requested, complete step 5 and write its final outcome before continuing. Then run `rasen archive "<name>" --intent-template --json`, complete an external intent including empty-handoff or probe-only intent, and run `rasen archive "<name>" --intent-file "<intent-path>" --dry-run --save-plan --json`. Inspect the immutable plan and require no blockers, capture its exact `planToken`, then run `rasen archive --apply-plan "<planToken>" --json --yes`. Retry recoverable outcomes only with the same token. This is the only archive bookkeeping path: never invoke an external spec-sync command, directly create an archive directory, move/delete the active change, write `archive.json`, or append evidence afterward.
