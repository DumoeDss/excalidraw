...
Govern durable product direction above individual Changes. Direction is opt-in: use it only when the user explicitly invokes it or accepts an explicit suggestion for work spanning multiple Changes, versions, horizons, projects, or recurring principle-level decisions.
